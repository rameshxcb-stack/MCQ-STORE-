...
// ==========================================
// ✅ SAFE DB CLIENT SINGLETON (EXPORTED)
// ==========================================
let _db = null;
...
export async function generateAndStoreMCQs({ subject, chapter, rawMCQsInput, evidenceText }) {
  ...
  const initialValidated = (rawMCQsInput || [])
    .map(validateRawMCQ)
    .filter(r => { 
      if (!r.valid) rejectedMCQs.push({ question: r.mcq?.question || 'Unknown', reason: `Structural: ${r.reason}` }); 
      return r.valid; 
    })
    .map(r => r.mcq);
