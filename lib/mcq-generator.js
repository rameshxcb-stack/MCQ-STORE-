...
// ==========================================
// ✅ SAFE DB CLIENT (No Crash on Import)
// ==========================================
let _db = null;
function getDb() {
  if (!_db) {
...
function getRandomKey(envVarName) {
  ...
  return activeKeys.length > 0 ? activeKeys[Math.floor(Math.random() * activeKeys.length)] : rawKeys[Math.floor(Math.random() * rawKeys.length)];
}
...
function safeJsonParseAndValidate(text, schema, contextName = 'API') {
  ...
  try {
    ...
    if (!validated.success) throw new Error(`Schema mismatch: ${validated.error.issues.map(i => i.message).join(', ')}`);
    return validated.data;
  } catch (err) { throw new Error(`[${contextName}] ${err.message}`); }
}
...
// ==========================================
// ✅ EVIDENCE RETRIEVER (Your Uploaded Code)
// ==========================================
async function retrieveEvidence(subject, chapter) {
  ...
  try {
    ...
    if (ddgRes.ok) {
      ...
      if (htmlText.toLowerCase().includes('captcha') || htmlText.includes('403')) {
        throw new Error("DDG returned CAPTCHA/Block.");
      }
  ...
  } catch (e) { console.warn("⚠️ DDG failed:", e.message); }
      ...
      if (jinaRes.ok) {
        ...
        if (fullText && fullText.length > 300 && !fullText.toLowerCase().includes('not found')) {
          const cleanedText = fullText
            .replace(/\[([^\]]+)\]\([^\)]+\)/g, '$1').replace(/https?:\/\/\S+/g, '').replace(/[#*`_~]/g, '').replace(/\s+/g, ' ').trim();
          console.log(`✅ Jina Reader parsed full PDF/HTML content for: ${subject} - ${chapter}`);
          return cleanedText.substring(0, 4000);
        }
      }
    } catch (e) { console.warn(`⚠️ Jina Reader failed: ${e.message}`); }
  }
  ...
  try {
    ...
    if (backupRes.ok) {
      ...
      if (text && text.length > 200) {
        const cleaned = text.replace(/\[([^\]]+)\]\([^\)]+\)/g, '$1').replace(/https?:\/\/\S+/g, '').replace(/[#*`_~]/g, '').replace(/\s+/g, ' ').trim();
        console.log(`✅ Backup: Jina Search content fetched for ${subject} - ${chapter}`);
        return cleaned.substring(0, 3500);
      }
    }
  } catch (e) { console.warn("⚠️ Backup search failed:", e.message); }
  ...
  try {
    ...
    if (wikiRes.ok) {
      ...
      if (wikiData.extract && wikiData.extract.length > 50) {
        console.log(`✅ Wikipedia Text for ${subject} - ${chapter}`);
        return wikiData.extract;
      }
    }
  } catch (e) { console.warn("⚠️ Wikipedia failed:", e.message); }

  return `Educational curriculum evidence for ${subject} - ${chapter}.`;
}

// ==========================================
// MODEL DISCOVERY & JUDGES (Core Generation)
// ==========================================
async function getAvailableGeminiModels(apiKey) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (models.length > 0) { cachedGeminiModels = { data: models, timestamp: Date.now() }; return models; }
    }
  } catch (e) { console.warn('⚠️ Gemini model fetch failed:', e.message); }
...
async function getAvailableDeepSeekModel(apiKey) {
  ...
  try {
    const res = await fetchWithRetry('https://api.deepseek.com/models', { method: 'GET', headers: { 'Authorization': `Bearer ${apiKey}` }, apiKeyUsed: apiKey }, 2);
    if (res.ok) {
      const data = await res.json();
      if (Array.isArray(data.data) && data.data.length > 0) { cachedDeepSeekModel = { data: data.data[0].id, timestamp: Date.now() }; return cachedDeepSeekModel.data; }
    }
  } catch (e) { console.warn('⚠️ DeepSeek model fetch failed:', e.message); }
  return 'deepseek-chat';
}

async function callGeminiWithFallback(prompt, bodyModifier = {}) {
  const initialKey = getRandomKey('GEMINI_KEYS');
  if (!initialKey) throw new Error('GEMINI_KEYS missing.');
  ...
  for (const model of availableModels) {
    ...
    try {
      const body = { contents: [{ parts: [{ text: prompt }] }], generationConfig: { response_mime_type: 'application/json', ...bodyModifier } };
      const res = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1/models/${model}:generateContent?key=${encodeURIComponent(currentKey)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), apiKeyUsed: currentKey });
      const data = await res.json();
      const text = data?.candidates?.[0]?.content?.parts?.[0]?.text;
      return safeJsonParseAndValidate(text, AIJudgeResultSchema, `Gemini (${model})`);
    } catch (err) { console.warn(`❌ Gemini ${model} failed: ${err.message}. Trying next...`); lastError = err; }
  }
  throw new Error(`All Gemini models failed. Last error: ${lastError?.message}`);
}

async function judgeWithGemini(question, options, answerText, explanation, evidence) {
  const prompt = `Analyze this MCQ for factual accuracy based strictly on the provided evidence. EVIDENCE: ${evidence} QUESTION: ${question} OPTIONS: ${JSON.stringify(options)} ANSWER: ${answerText} EXPLANATION: ${explanation} Return JSON strictly: {"passed": boolean, "confidence": number (0-100), "options_quality": number (0-100), "question_quality": number (0-100), "explanation_valid": boolean, "contradiction": boolean, "evidence_sufficient": boolean, "unique_correct_option": boolean, "ambiguous": boolean}`;
...
async function judgeWithDeepSeek(question, options, answerText, explanation, evidence) {
  ...
  if (!apiKey) throw new Error('DEEPSEEK_KEYS missing.');
  ...
  const res = await fetchWithRetry('https://api.deepseek.com/v1/chat/completions', {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` }, body: JSON.stringify({ model: activeModel, messages: [{ role: 'user', content: prompt }], response_format: { type: 'json_object' } }), apiKeyUsed: apiKey
  });
...
function calculateQualityScore(evidence, judgeResult, modelAgreementScore) {
  const baseScore = (Number(judgeResult.question_quality)||0 * 0.35) + (Number(judgeResult.options_quality)||0 * 0.35) + (Number(judgeResult.confidence)||0 * 0.20) + (Number(modelAgreementScore) * 0.10);
  return Math.round(Math.min(100, Math.max(0, baseScore)));
}

function validateRawMCQ(mcq) {
  const result = RawMCQSchema.safeParse(mcq);
  if (!result.success) return { valid: false, reason: result.error.issues.map(i => i.message).join('; ') };
  const data = result.data;
  const uniqueOpts = new Set(data.options.map(o => normalizeMCQText(o)));
  if (uniqueOpts.size !== 4) return { valid: false, reason: 'Duplicate options' };
  let answerIdx = -1;
  if (typeof data.answer === 'number') answerIdx = data.answer;
  else {
  ...
  if (answerIdx === -1) return { valid: false, reason: 'Answer index not mapped' };
  return { valid: true, mcq: { question: data.question.trim(), options: data.options.map(o => o.trim()), answer: answerIdx, difficulty: data.difficulty.trim(), explanation: data.explanation.trim() } };
}
...
export async function generateAndStoreMCQs({ subject, chapter, rawMCQsInput, evidenceText }) {
  // ✅ Safe DB Connection (कभी crash नहीं होगा)
  const db = getDb();

  const safeEvidence = typeof evidenceText === 'string' ? evidenceText.slice(0, 10000) : '';
  const rejectedMCQs = [];
  const initialValidated = (rawMCQsInput || []).map(validateRawMCQ).filter(r => { if (!r.valid) rejectedMCQs.push({ question: r.mcq?.question || 'Unknown', reason: `Structural: ${r.reason}` }); return r.valid; }).map(r => r.mcq);
  if (initialValidated.length === 0) return { success: false, count: 0, rejectedDetails: rejectedMCQs };
      ...
      try {
        ...
        if (!geminiResult.passed || geminiResult.confidence < 90 || geminiResult.ambiguous || !geminiResult.evidence_sufficient) {
          try {
            ...
            if (geminiResult.passed !== deepResult.passed) { rejectedMCQs.push({ question: mcq.question, reason: 'Model disagreement' }); return null; }
            agreement = 100;
            if (geminiResult.passed && deepResult.passed) final = { ...geminiResult, confidence: Math.min(geminiResult.confidence, deepResult.confidence) };
          } catch (e) { rejectedMCQs.push({ question: mcq.question, reason: 'DeepSeek failed' }); return null; }
        }
        if (!final.passed || final.contradiction || !final.explanation_valid || !final.evidence_sufficient || !final.unique_correct_option || final.ambiguous) {
          rejectedMCQs.push({ question: mcq.question, reason: final.reason || 'Quality gate failed' }); return null;
        }
        const score = calculateQualityScore(safeEvidence, final, agreement);
        if (score < QUALITY_GATE) { rejectedMCQs.push({ question: mcq.question, reason: `Score ${score} < ${QUALITY_GATE}` }); return null; }
        return { mcq, score };
      } catch (e) { rejectedMCQs.push({ question: mcq.question, reason: `Error: ${e.message}` }); return null; }
    });
  ...
  if (approvedMCQs.length === 0) return { success: false, count: 0, rejectedDetails: rejectedMCQs };
  ...
  for (const item of uniqueItems) {
    const id = randomUUID();
    const hash = createHash('sha256').update(normalizeMCQText(item.mcq.question)).digest('hex');
    args.push(id, subject, chapter, item.mcq.difficulty, item.mcq.question, item.mcq.options[0], item.mcq.options[1], item.mcq.options[2], item.mcq.options[3], item.mcq.answer, item.mcq.explanation, hash, item.score, Date.now());
  }
  ...
  return { success: true, count: uniqueItems.length, rejectedTotal: rejectedMCQs.length, rejectedDetails: rejectedMCQs };
}
