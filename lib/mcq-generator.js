...
export function getDb() {
  ...
  if (!rawUrl) throw new Error('TURSO_DATABASE_URL is missing.');
  if (!rawToken) throw new Error('TURSO_AUTH_TOKEN is missing.');

  // ✅ Clean accidental formatting from Vercel environment variable
  const cleanToken = rawToken
    .replace(/^Bearer\s+/i, '')        // Remove "Bearer " prefix if any
  ...
  if (!cleanToken) throw new Error('TURSO_AUTH_TOKEN is empty after sanitization.');
  if (!cleanUrl) throw new Error('TURSO_DATABASE_URL is empty after sanitization.');

  _db = createClient({
    url: cleanUrl,      // keep libsql:// as @libsql/client expects it
